(define-char
  '((name		. "BLACK SQUARE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (japanese-jisx0208-1978	. #x2223)	; ø»½ 
    (chinese-gb2312		. #x2176)	; ø»„‡™
    (japanese-jisx0208		. #x2223)	; ø»†‘¨
    (korean-ksc5601		. #x2161)	; ø»ˆ›Œ
    (chinese-cns11643-1		. #x217C)	; ø»‘ƒ·
    (japanese-jisx0208-1990	. #x2223)	; â– 
    (ucs			. #x25A0)	; â– 
    ))
(define-char
  '((name		. "WHITE SQUARE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (japanese-jisx0208-1978	. #x2222)	; ø»½Ÿ
    (chinese-gb2312		. #x2175)	; ø»„‡˜
    (japanese-jisx0208		. #x2222)	; ø»†‘§
    (korean-ksc5601		. #x2160)	; ø»ˆ›‹
    (chinese-cns11643-1		. #x217B)	; ø»‘ƒ¶
    (japanese-jisx0208-1990	. #x2222)	; â–¡
    (ucs			. #x25A1)	; â–¡
    ))
(define-char
  '((name		. "WHITE SQUARE WITH ROUNDED CORNERS")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25A2)	; â–¢
    ))
(define-char
  '((name		. "WHITE SQUARE CONTAINING BLACK SMALL SQUARE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (korean-ksc5601	. #x2243)	; ø»ˆœŒ
    (ucs		. #x25A3)	; â–£
    ))
(define-char
  '((name		. "SQUARE WITH HORIZONTAL FILL")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (korean-ksc5601	. #x2247)	; ø»ˆœ
    (ucs		. #x25A4)	; â–¤
    ))
(define-char
  '((name		. "SQUARE WITH VERTICAL FILL")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (korean-ksc5601	. #x2248)	; ø»ˆœ‘
    (ucs		. #x25A5)	; â–¥
    ))
(define-char
  '((name		. "SQUARE WITH ORTHOGONAL CROSSHATCH FILL")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (korean-ksc5601	. #x224B)	; ø»ˆœ”
    (ucs		. #x25A6)	; â–¦
    ))
(define-char
  '((name		. "SQUARE WITH UPPER LEFT TO LOWER RIGHT FILL")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (korean-ksc5601	. #x224A)	; ø»ˆœ“
    (ucs		. #x25A7)	; â–§
    ))
(define-char
  '((name		. "SQUARE WITH UPPER RIGHT TO LOWER LEFT FILL")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (korean-ksc5601	. #x2249)	; ø»ˆœ’
    (ucs		. #x25A8)	; â–¨
    ))
(define-char
  '((name		. "SQUARE WITH DIAGONAL CROSSHATCH FILL")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (korean-ksc5601	. #x224C)	; ø»ˆœ•
    (ucs		. #x25A9)	; â–©
    ))
(define-char
  '((name		. "BLACK SMALL SQUARE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25AA)	; â–ª
    ))
(define-char
  '((name		. "WHITE SMALL SQUARE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25AB)	; â–«
    ))
(define-char
  '((name		. "BLACK RECTANGLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25AC)	; â–¬
    ))
(define-char
  '((name		. "WHITE RECTANGLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25AD)	; â–­
    ))
(define-char
  '((name		. "BLACK VERTICAL RECTANGLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25AE)	; â–®
    ))
(define-char
  '((name		. "WHITE VERTICAL RECTANGLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25AF)	; â–¯
    ))
(define-char
  '((name		. "BLACK PARALLELOGRAM")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25B0)	; â–°
    ))
(define-char
  '((name		. "WHITE PARALLELOGRAM")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (japanese-jisx0213-1	. #x266D)	; â–±
    (ucs			. #x25B1)	; â–±
    ))
(define-char
  '((name		. "BLACK UP-POINTING TRIANGLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (japanese-jisx0208-1978	. #x2225)	; ø»½¢
    (chinese-gb2312		. #x2178)	; ø»„‡›
    (japanese-jisx0208		. #x2225)	; ø»†‘ª
    (korean-ksc5601		. #x2163)	; ø»ˆ›Ž
    (chinese-cns11643-1		. #x2175)	; ø»‘ƒ°
    (japanese-jisx0208-1990	. #x2225)	; â–²
    (ucs			. #x25B2)	; â–²
    ))
(define-char
  '((name		. "WHITE UP-POINTING TRIANGLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (japanese-jisx0208-1978	. #x2224)	; ø»½¡
    (chinese-gb2312		. #x2177)	; ø»„‡š
    (japanese-jisx0208		. #x2224)	; ø»†‘©
    (korean-ksc5601		. #x2162)	; ø»ˆ›
    (chinese-cns11643-1		. #x2174)	; ø»‘ƒ¯
    (japanese-jisx0208-1990	. #x2224)	; â–³
    (ucs			. #x25B3)	; â–³
    ))
(define-char
  '((name		. "BLACK UP-POINTING SMALL TRIANGLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25B4)	; â–´
    ))
(define-char
  '((name		. "WHITE UP-POINTING SMALL TRIANGLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25B5)	; â–µ
    ))
(define-char
  '((name		. "BLACK RIGHT-POINTING TRIANGLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (korean-ksc5601		. #x223A)	; ø»ˆœƒ
    (japanese-jisx0213-1	. #x2322)	; â–¶
    (ucs			. #x25B6)	; â–¶
    ))
(define-char
  '((name		. "WHITE RIGHT-POINTING TRIANGLE")
    (general-category	symbol math) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (korean-ksc5601		. #x2239)	; ø»ˆœ‚
    (japanese-jisx0213-1	. #x2321)	; â–·
    (ucs			. #x25B7)	; â–·
    ))
(define-char
  '((name		. "BLACK RIGHT-POINTING SMALL TRIANGLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25B8)	; â–¸
    ))
(define-char
  '((name		. "WHITE RIGHT-POINTING SMALL TRIANGLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25B9)	; â–¹
    ))
(define-char
  '((name		. "BLACK RIGHT-POINTING POINTER")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25BA)	; â–º
    ))
(define-char
  '((name		. "WHITE RIGHT-POINTING POINTER")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25BB)	; â–»
    ))
(define-char
  '((name		. "BLACK DOWN-POINTING TRIANGLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (japanese-jisx0208-1978	. #x2227)	; ø»½¤
    (japanese-jisx0208		. #x2227)	; ø»†‘¬
    (korean-ksc5601		. #x2165)	; ø»ˆ›
    (chinese-cns11643-1		. #x217E)	; ø»‘ƒ¹
    (japanese-jisx0208-1990	. #x2227)	; â–¼
    (ucs			. #x25BC)	; â–¼
    ))
(define-char
  '((name		. "WHITE DOWN-POINTING TRIANGLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (japanese-jisx0208-1978	. #x2226)	; ø»½£
    (japanese-jisx0208		. #x2226)	; ø»†‘«
    (korean-ksc5601		. #x2164)	; ø»ˆ›
    (chinese-cns11643-1		. #x217D)	; ø»‘ƒ¸
    (japanese-jisx0208-1990	. #x2226)	; â–½
    (ucs			. #x25BD)	; â–½
    ))
(define-char
  '((name		. "BLACK DOWN-POINTING SMALL TRIANGLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25BE)	; â–¾
    ))
(define-char
  '((name		. "WHITE DOWN-POINTING SMALL TRIANGLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25BF)	; â–¿
    ))
(define-char
  '((name		. "BLACK LEFT-POINTING TRIANGLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (korean-ksc5601		. #x2238)	; ø»ˆœ
    (japanese-jisx0213-1	. #x2324)	; â—€
    (ucs			. #x25C0)	; â—€
    ))
(define-char
  '((name		. "WHITE LEFT-POINTING TRIANGLE")
    (general-category	symbol math) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (korean-ksc5601		. #x2237)	; ø»ˆœ€
    (japanese-jisx0213-1	. #x2323)	; â—
    (ucs			. #x25C1)	; â—
    ))
(define-char
  '((name		. "BLACK LEFT-POINTING SMALL TRIANGLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25C2)	; â—‚
    ))
(define-char
  '((name		. "WHITE LEFT-POINTING SMALL TRIANGLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25C3)	; â—ƒ
    ))
(define-char
  '((name		. "BLACK LEFT-POINTING POINTER")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25C4)	; â—„
    ))
(define-char
  '((name		. "WHITE LEFT-POINTING POINTER")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25C5)	; â—…
    ))
(define-char
  '((name		. "BLACK DIAMOND")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (japanese-jisx0208-1978	. #x2221)	; ø»½ž
    (chinese-gb2312		. #x2174)	; ø»„‡—
    (japanese-jisx0208		. #x2221)	; ø»†‘¦
    (korean-ksc5601		. #x215F)	; ø»ˆ›Š
    (chinese-cns11643-1		. #x217A)	; ø»‘ƒµ
    (japanese-jisx0208-1990	. #x2221)	; â—†
    (ucs			. #x25C6)	; â—†
    ))
(define-char
  '((name		. "WHITE DIAMOND")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (japanese-jisx0208-1978	. #x217E)	; ø»½
    (chinese-gb2312		. #x2173)	; ø»„‡–
    (japanese-jisx0208		. #x217E)	; ø»†‘¥
    (korean-ksc5601		. #x215E)	; ø»ˆ›‰
    (chinese-cns11643-1		. #x2179)	; ø»‘ƒ´
    (japanese-jisx0208-1990	. #x217E)	; â—‡
    (ucs			. #x25C7)	; â—‡
    ))
(define-char
  '((name		. "WHITE DIAMOND CONTAINING BLACK SMALL DIAMOND")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (korean-ksc5601	. #x2242)	; ø»ˆœ‹
    (ucs		. #x25C8)	; â—ˆ
    ))
(define-char
  '((name		. "FISHEYE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (japanese-jisx0213-1	. #x233B)	; â—‰
    (ucs			. #x25C9)	; â—‰
    ))
(define-char
  '((name		. "LOZENGE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25CA)	; â—Š
    ))
(define-char
  '((name		. "WHITE CIRCLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (japanese-jisx0208-1978	. #x217B)	; ø»½š
    (chinese-gb2312		. #x2170)	; ø»„‡“
    (japanese-jisx0208		. #x217B)	; ø»†‘¢
    (korean-ksc5601		. #x215B)	; ø»ˆ›†
    (chinese-cns11643-1		. #x2172)	; ø»‘ƒ­
    (japanese-jisx0208-1990	. #x217B)	; â—‹
    (ucs			. #x25CB)	; â—‹
    ))
(define-char
  '((name		. "DOTTED CIRCLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25CC)	; â—Œ
    ))
(define-char
  '((name		. "CIRCLE WITH VERTICAL FILL")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25CD)	; â—
    ))
(define-char
  '((name		. "BULLSEYE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (japanese-jisx0208-1978	. #x217D)	; ø»½œ
    (chinese-gb2312		. #x2172)	; ø»„‡•
    (japanese-jisx0208		. #x217D)	; ø»†‘¤
    (korean-ksc5601		. #x215D)	; ø»ˆ›ˆ
    (chinese-cns11643-1		. #x2176)	; ø»‘ƒ±
    (japanese-jisx0208-1990	. #x217D)	; â—Ž
    (ucs			. #x25CE)	; â—Ž
    ))
(define-char
  '((name		. "BLACK CIRCLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (japanese-jisx0208-1978	. #x217C)	; ø»½›
    (chinese-gb2312		. #x2171)	; ø»„‡”
    (japanese-jisx0208		. #x217C)	; ø»†‘£
    (korean-ksc5601		. #x215C)	; ø»ˆ›‡
    (chinese-cns11643-1		. #x2173)	; ø»‘ƒ®
    (japanese-jisx0208-1990	. #x217C)	; â—
    (ucs			. #x25CF)	; â—
    ))
(define-char
  '((name		. "CIRCLE WITH LEFT HALF BLACK")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (korean-ksc5601		. #x2244)	; ø»ˆœ
    (japanese-jisx0213-1	. #x2867)	; â—
    (ucs			. #x25D0)	; â—
    ))
(define-char
  '((name		. "CIRCLE WITH RIGHT HALF BLACK")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (korean-ksc5601		. #x2245)	; ø»ˆœŽ
    (japanese-jisx0213-1	. #x2868)	; â—‘
    (ucs			. #x25D1)	; â—‘
    ))
(define-char
  '((name		. "CIRCLE WITH LOWER HALF BLACK")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (japanese-jisx0213-1	. #x2869)	; â—’
    (ucs			. #x25D2)	; â—’
    ))
(define-char
  '((name		. "CIRCLE WITH UPPER HALF BLACK")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (japanese-jisx0213-1	. #x286A)	; â—“
    (ucs			. #x25D3)	; â—“
    ))
(define-char
  '((name		. "CIRCLE WITH UPPER RIGHT QUADRANT BLACK")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25D4)	; â—”
    ))
(define-char
  '((name		. "CIRCLE WITH ALL BUT UPPER LEFT QUADRANT BLACK")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25D5)	; â—•
    ))
(define-char
  '((name		. "LEFT HALF BLACK CIRCLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25D6)	; â—–
    ))
(define-char
  '((name		. "RIGHT HALF BLACK CIRCLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25D7)	; â——
    ))
(define-char
  '((name		. "INVERSE BULLET")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25D8)	; â—˜
    ))
(define-char
  '((name		. "INVERSE WHITE CIRCLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25D9)	; â—™
    ))
(define-char
  '((name		. "UPPER HALF INVERSE WHITE CIRCLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25DA)	; â—š
    ))
(define-char
  '((name		. "LOWER HALF INVERSE WHITE CIRCLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25DB)	; â—›
    ))
(define-char
  '((name		. "UPPER LEFT QUADRANT CIRCULAR ARC")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25DC)	; â—œ
    ))
(define-char
  '((name		. "UPPER RIGHT QUADRANT CIRCULAR ARC")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25DD)	; â—
    ))
(define-char
  '((name		. "LOWER RIGHT QUADRANT CIRCULAR ARC")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25DE)	; â—ž
    ))
(define-char
  '((name		. "LOWER LEFT QUADRANT CIRCULAR ARC")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25DF)	; â—Ÿ
    ))
(define-char
  '((name		. "UPPER HALF CIRCLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25E0)	; â— 
    ))
(define-char
  '((name		. "LOWER HALF CIRCLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25E1)	; â—¡
    ))
(define-char
  '((name		. "BLACK LOWER RIGHT TRIANGLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (chinese-cns11643-1 . #x2348)	; ø»‘…¿
    (ucs		. #x25E2)	; â—¢
    ))
(define-char
  '((name		. "BLACK LOWER LEFT TRIANGLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (chinese-cns11643-1 . #x2349)	; ø»‘†€
    (ucs		. #x25E3)	; â—£
    ))
(define-char
  '((name		. "BLACK UPPER LEFT TRIANGLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (chinese-cns11643-1 . #x234B)	; ø»‘†‚
    (ucs		. #x25E4)	; â—¤
    ))
(define-char
  '((name		. "BLACK UPPER RIGHT TRIANGLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (chinese-cns11643-1 . #x234A)	; ø»‘†
    (ucs		. #x25E5)	; â—¥
    ))
(define-char
  '((name		. "WHITE BULLET")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (japanese-jisx0213-1	. #x233F)	; â—¦
    (ucs			. #x25E6)	; â—¦
    ))
(define-char
  '((name		. "SQUARE WITH LEFT HALF BLACK")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25E7)	; â—§
    ))
(define-char
  '((name		. "SQUARE WITH RIGHT HALF BLACK")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25E8)	; â—¨
    ))
(define-char
  '((name		. "SQUARE WITH UPPER LEFT DIAGONAL HALF BLACK")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25E9)	; â—©
    ))
(define-char
  '((name		. "SQUARE WITH LOWER RIGHT DIAGONAL HALF BLACK")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25EA)	; â—ª
    ))
(define-char
  '((name		. "WHITE SQUARE WITH VERTICAL BISECTING LINE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25EB)	; â—«
    ))
(define-char
  '((name		. "WHITE UP-POINTING TRIANGLE WITH DOT")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25EC)	; â—¬
    ))
(define-char
  '((name		. "UP-POINTING TRIANGLE WITH LEFT HALF BLACK")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25ED)	; â—­
    ))
(define-char
  '((name		. "UP-POINTING TRIANGLE WITH RIGHT HALF BLACK")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25EE)	; â—®
    ))
(define-char
  '((name		. "LARGE CIRCLE")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (japanese-jisx0208		. #x227E)	; ø»†“ƒ
    (japanese-jisx0208-1990	. #x227E)	; â—¯
    (ucs			. #x25EF)	; â—¯
    ))
(define-char
  '((name		. "WHITE SQUARE WITH UPPER LEFT QUADRANT")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25F0)	; â—°
    ))
(define-char
  '((name		. "WHITE SQUARE WITH LOWER LEFT QUADRANT")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25F1)	; â—±
    ))
(define-char
  '((name		. "WHITE SQUARE WITH LOWER RIGHT QUADRANT")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25F2)	; â—²
    ))
(define-char
  '((name		. "WHITE SQUARE WITH UPPER RIGHT QUADRANT")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25F3)	; â—³
    ))
(define-char
  '((name		. "WHITE CIRCLE WITH UPPER LEFT QUADRANT")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25F4)	; â—´
    ))
(define-char
  '((name		. "WHITE CIRCLE WITH LOWER LEFT QUADRANT")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25F5)	; â—µ
    ))
(define-char
  '((name		. "WHITE CIRCLE WITH LOWER RIGHT QUADRANT")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25F6)	; â—¶
    ))
(define-char
  '((name		. "WHITE CIRCLE WITH UPPER RIGHT QUADRANT")
    (general-category	symbol other) ; Informative Category
    (bidi-category	. "ON")
    (mirrored		. nil)
    (ucs		. #x25F7)	; â—·
    ))
