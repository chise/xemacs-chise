;; -*- coding: utf-8-mcs-er -*-
(define-char
  '((shuowen-radical	. 113)	; 𥄕
    (=shuowen-jiguge	. 11907)	; &SW-JIGUGE-11907;
    (<-Small-Seal@shuowen
     ((=ucs		  . #x25115)	; 𥄕
      ))
    (->subsumptive
     ((===shuowen-jiguge5 . 11907)	; &SW-JIGUGE5-11907;
      (<-Small-Seal@shuowen/mpsuzuki
       ((=ucs		    . #x82DC)	; 苜
	))
      )
     ((===shuowen-jiguge4 . 11607)	; &SW-JIGUGE4-11607;
      (<-Small-Seal@shuowen/mpsuzuki
       ((=ucs		    . #x82DC)	; 苜
	))
      ))
    ))
(define-char
  '((shuowen-radical	. 113)	; 𥄕
    (=shuowen-jiguge	. 11908)	; &SW-JIGUGE-11908;
    (<-Small-Seal@shuowen
     ((=ucs		  . #x77A2)	; 瞢
      ))
    (->subsumptive
     ((===shuowen-jiguge5 . 11908)	; &SW-JIGUGE5-11908;
      )
     ((===shuowen-jiguge4 . 11608)	; &SW-JIGUGE4-11608;
      ))
    ))
(define-char
  '((shuowen-radical	. 113)	; 𥄕
    (=shuowen-jiguge	. 11909)	; &SW-JIGUGE-11909;
    (<-Small-Seal@shuowen
     ((=ucs		  . #x242BE)	; 𤊾
      ))
    (->subsumptive
     ((===shuowen-jiguge5 . 11909)	; &SW-JIGUGE5-11909;
      )
     ((===shuowen-jiguge4 . 11609)	; &SW-JIGUGE4-11609;
      ))
    ))
(define-char
  '((shuowen-radical	. 113)	; 𥄕
    (=shuowen-jiguge	. 11910)	; &SW-JIGUGE-11910;
    (<-Small-Seal@shuowen
     ((=ucs		  . #x8511)	; 蔑
      ))
    (->subsumptive
     ((===shuowen-jiguge5 . 11910)	; &SW-JIGUGE5-11910;
      )
     ((===shuowen-jiguge4 . 11610)	; &SW-JIGUGE4-11610;
      ))
    ))
